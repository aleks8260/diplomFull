...
// Обрезание строки
function mbCutString($string, $length, $postfix = '...', $endcoding = 'UTF-8'){
	if(mb_strlen($string, $endcoding) <= $length){
		return $string;
	}
	$temp = mb_substr($string, 0, $length, $endcoding);
	// ищем положение пробела
	$spacenPosition = mb_strripos($temp, " ", 0, $endcoding);
